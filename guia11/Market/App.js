...
const APIUSER_URI = 'http://192.168.0.42/g11/apiusuario.php' ;
const API_URI = 'http://192.168.0.42/g11/api.php';
class PantallaInicio extends React.Component {
  ...
  Entrar() {
    if (!!this.state.usuario && !!this.state.contrasena) {
      fetch(APIUSER_URI + '?comando=autenticar&usuario=' + this.state.usuario + ' & contrasena=' + this.state.contrasena, 
      { method: 'GET' })
...
class PaginaDetalle extends React.Component {
  ...
  Actualizar() {
    fetch(API_URI + '?comando=editar&nombre=' + this.state.nombre
  ...
  Eliminar() {
    fetch(API_URI + ' ?comando=eliminar&id=' + this.state.id, {
      method: 'GET'
    })
      .then((response) => response.json())
      .then((responseJson) => {
...
class PaginaAgregar extends React.Component {
  ...
  Guardar() {
    fetch(API_URI + '?comando=agregar&nombre=' +
      this.state.nombre
      + '&descripcion=' + this.state.descripcion
      + '&cantidad=' + this.state.cantidad
      + '&preciodecosto=' + this.state.preciodecosto
